...
export default function LoansPage({ loans, onSaveLoan, onUpdateLoan, currentStaff, currentStore }) {
  ...
  useEffect(() => {
    if (showForm || settleLoanId || previewLoan) {
  ...
  }, [showForm, settleLoanId, previewLoan]);

  const filtered = loans.filter(l => {
    if (filter === 'Pending Approvals') {
      if (l.status !== 'SettlePending') return false;
  ...
  const handleSettle = () => {
    const loan = loans.find(l => l.id === settleLoanId || l._id === settleLoanId);
    if (!loan) return;
    ...
    const updatedLoan = {
      ...loan,
      status: currentStaff?.role === 'Admin' ? 'Closed' : 'SettlePending',
      closingDate: now,
    ...
    setSuccess(currentStaff?.role === 'Admin' ? 'Loan settled successfully!' : 'Settlement request submitted for Admin approval!');
    setTimeout(() => setSuccess(''), 3000);
  ...
  const handleAdminApproveSettle = (loan) => {
    ...
    onUpdateLoan(updatedLoan);
    setSuccess('Gold Loan settlement approved!');
    setTimeout(() => setSuccess(''), 3000);
  };
  ...
  const handleAdminRejectSettle = (loan) => {
    ...
    onUpdateLoan(updatedLoan);
    setSuccess('Gold Loan settlement request rejected!');
    setTimeout(() => setSuccess(''), 3000);
  };
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2">
            {['All', 'Active', 'Closed', ...(currentStaff?.role === 'Admin' || loans.some(l => l.status === 'SettlePending') ? ['Pending Approvals'] : [])].map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all
                  ${filter === f ? 'bg-amber-500 text-white shadow-sm' : 'bg-gray-100 text-gray-600 border border-gray-200 hover:bg-gray-200'}`}
              >
                {f === 'All' ? 'All Loans' : f}
              </button>
            ))}
                ...
                {loan.status === 'SettlePending' && (
                  currentStaff?.role === 'Admin' ? (
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => handleAdminApproveSettle(loan)}
                        className="px-3.5 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-450 text-white text-xs font-semibold transition-all shadow-sm"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => handleAdminRejectSettle(loan)}
                        className="px-3.5 py-1.5 rounded-lg bg-red-500 hover:bg-red-450 text-white text-xs font-semibold transition-all shadow-sm"
                      >
                        Reject
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-gray-800">
          <div className="w-full max-w-lg bg-white border border-gray-200 rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="text-xs text-gray-500 font-semibold mb-1.5 block uppercase tracking-wider">Damage Percentage *</label>
                <input type="text" value={form.damagePercentage} onChange={e => setForm(p => ({ ...p, damagePercentage: e.target.value }))} className="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-2.5 text-gray-800 text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-100 focus:outline-none" />
              </div>
              <div>
