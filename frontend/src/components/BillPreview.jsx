...
function BillContent({ bill }) {
  // Resolve customer details safely supporting flat and nested structures
  ...
  // Resolve totals safely supporting both frontend & backend property mappings
  ...
  const gstAmount = bill.totalGST ?? bill.gstAmount ?? 0;
  const finalTotal = bill.finalTotal ?? bill.totalAmount ?? 0;
    ...
    <div className="font-mono text-sm bg-white text-gray-900 p-8 max-w-[400px] mx-auto">
      ...
      <div className="border-t border-b border-gray-400 py-3 mb-3">
        ...
        {(bill.items || []).map((item, i) => {
          // Resolve item properties safely
          const itemGoldValue = item.goldValue ?? (item.weight * (item.goldRate || 0) * (item.quantity || 1));
          const itemMakingCharge = item.makingCharge || 0;
          const itemStoneCharge = item.stoneCharge || 0;
          const itemSubtotal = item.subtotal ?? (itemGoldValue + itemMakingCharge + itemStoneCharge);
          const itemGst = item.gstAmount ?? (itemSubtotal * 0.03);
          ...
          return (
            <div key={i} className="mb-3 text-xs">
              <div className="flex">
                ...
                <span className="w-24 text-right">{formatCurrency(itemGoldValue)}</span>
              </div>
              {itemMakingCharge > 0 && (
                <div className="flex text-gray-600 pl-2">
                  <span className="flex-1">Making Charge</span>
                  <span className="w-24 text-right">{formatCurrency(itemMakingCharge)}</span>
                </div>
              )}
              {itemStoneCharge > 0 && (
                <div className="flex text-gray-600 pl-2">
                  <span className="flex-1">Stone Charge</span>
                  <span className="w-24 text-right">{formatCurrency(itemStoneCharge)}</span>
                </div>
              )}
              <div className="flex text-gray-600 pl-2">
                <span className="flex-1">GST (3%)</span>
                <span className="w-24 text-right">{formatCurrency(itemGst)}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="text-xs space-y-1 mb-4">
        <div className="flex justify-between">
          ...
          <span>{formatCurrency(goldValue)}</span>
        </div>
        {makingCharge > 0 && (
          <div className="flex justify-between">
        ...
        {stoneCharge > 0 && (
          <div className="flex justify-between">
            ...
            <span>{formatCurrency(stoneCharge)}</span>
          </div>
        )}
        <div className="flex justify-between">
          <span>GST (3%)</span>
